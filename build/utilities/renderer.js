 ...
 */
function raycast(x, y, heading, level, range = 50) {
    let curx = x, cury = y;
    const radian = (heading / 180) * Math.PI;
    let slopex = Math.sin(radian) * 0.25;
    let slopey = -Math.cos(radian) * 0.25;
    for (let step = 0; step < range; step += 0.25) {
        curx += slopex;
        cury += slopey;
        for (let row = -1; row <= 1; row++) {
            for (let column = -1; column <= 1; column++) {
                let truex = Math.floor(curx + column);
                let truey = Math.floor(cury + row);
                let segmentx = Math.floor((curx + column) / 10);
                let segmenty = Math.floor((cury + row) / 10);
                let tilex = Math.floor(curx % 10);
                let tiley = Math.floor(cury % 10);
                if (segmentx < 0)
 ...
 */
export function scanLevel(player, level, fov = 100, density = 4) {
    let distances = [];
    for (let x = -125; x <= 125; x += density) {
        let dir = Math.atan(x / dv) * (180 / Math.PI);
        let raw = raycast(player.position.x, player.position.y, player.position.heading + dir, level);
        if (raw === null) {
            distances.push(null);
            continue;
        }
...
export function makeScan(scan, factor = 300, width = 500) {
    ...
    scans.sort((a, b) => a.distance < b.distance ? 1 : a.distance > b.distance ? -1 : 0);
    return {
        move: move,
        scans: scans
    };
}
export function renderScene(ctx, player, scan, entities) {
    let scans = scan.scans;
    for (const i in entities) {
        let entitydepth = entities[i].getEntityPosition(player)[1];
